...
class MonitorChanges(PatternMatchingEventHandler):
    ...
    def on_created(self, event):
        ...
        if ".zip" in event.src_path:
            ...
            # PCODE_FILE = Path(str(T).replace('.csv', '.pcode'))
            # with open(str(PCODE_FILE), 'w') as f:
            #     f.write("THIS IS TEST P-CODE\n")
            ...
            cmd = f""" ssh -p {server['port']} {server['username']}@{server['ip']} "ssh -p {pi['port']} {pi['username']}@{pi['ip']} 'cat /home/{pi['username']}/temp.txt > {pi['remote_path']}/{Path(MUSIC_FILE).name}'" """.strip()
            logger.info(cmd)
            os.system(cmd)
            time.sleep(2)
