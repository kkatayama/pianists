    ...
    for page in doc:
        ...
        for i, r in enumerate(new_rects):
            ...
            if pix.height < pix.width:
                f_all = str(BASE_PATH.joinpath(f"draw_raw/{count}.png"))
                f_top = str(BASE_PATH.joinpath(f"draw_crop/{count}.png"))
                count += 1
                f_btm = str(BASE_PATH.joinpath(f"draw_crop/{count}.png"))
                pix.save(f_all)
...
if __name__ == '__main__':
    ...
    ap.add_argument("base_path", help="the base folder to export the drawings to base_path/{draw_raw, draw_crop}")
    args = ap.parse_args()
    ...
    BASE_PATH.joinpath("draw_raw").mkdir(exist_ok=True)
    BASE_PATH.joinpath("draw_crop").mkdir(exist_ok=True)
